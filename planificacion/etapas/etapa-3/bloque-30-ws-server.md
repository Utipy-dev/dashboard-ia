...
- **Autenticación por token.** Al conectar, el cliente envía `?token=<hash>` en la URL del handshake. El servidor verifica contra `src/servidor/data/config.json`. Si no coincide, cierra la conexión. Razón: evitar que cualquier dispositivo de la red local espíe o inyecte.
- **Protocolo JSON tipado.** Todos los mensajes WS tienen forma `{ tipo, payload, ts }`. Los tipos se definen en `src/servidor/src/ws/tipos.js` como constantes. Razón: los consumidores (móvil, cola) deserializan el mismo protocolo sin asumir nada.
...
- **Plugin `src/servidor/src/ws/plugin.js`:**
...
- **`src/servidor/src/ws/broadcaster.js`:**
...
- `src/servidor/src/ws/plugin.js` — nuevo
- `src/servidor/src/ws/broadcaster.js` — nuevo
- `src/servidor/src/ws/tipos.js` — nuevo
- `src/servidor/src/config.js` — añadir cert/key/token
- `src/servidor/src/index.js` — registrar plugin WS
- `src/servidor/src/services/storage.js` — llamar broadcaster en cada write
- `src/scripts/generar-cert.sh` — nuevo
- `src/dashboard/ws-client.js` — nuevo
...
- [ ] `src/scripts/generar-cert.sh` genera el certificado sin errores en Windows (Git Bash) y macOS
...
- **El certificado va a `.gitignore`.** `src/servidor/data/certs/` contiene claves privadas. Nunca subir al repo.
